...
Public Class Form1
    ...
    Public Ventas As XmlDocument = New XmlDocument
    Public XmlElement As XmlElement
            ...
            If MessageBox.Show("¿Desea realizar la comanda?", "Titulo", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = DialogResult.OK Then
                ...
                If MessageBox.Show("¿Desea imprimir el ticket?", "Titulo", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = DialogResult.OK Then
                    Try
                        ...
                        Try
                            Ventas.Load("../../Ventas.xml")
                            node = Ventas.SelectSingleNode("//Ventas")
                            If node.LastChild.Attributes.GetNamedItem("id").Value <> Nothing Then
                                id = node.LastChild.Attributes.GetNamedItem("id").Value + 1
                            End If
                        ...
                        End Try
                        XmlElement = añadirNuevo(id, UniMesTot, Ventas)
                        Ventas = XmlElement.OwnerDocument
                        Ventas.DocumentElement.AppendChild(XmlElement)
                        SaveXML(Ventas)
                        MsgBox("Xml generado con el ticket")
    ...
    Public Function crearXml()
        Ventas.PreserveWhitespace = True
        Try
            Ventas.Load("../../Ventas.xml")
    ...
    Private Function generateXMLString() As String
        Dim xml As String = "<?xml version=""1.0""?>" & ControlChars.NewLine &
            "<Ventas>" & ControlChars.NewLine &
            "   <venta id = ""1"" fecha = ""2019/02/11"">" & ControlChars.NewLine &
            "       <unidades>00</unidades>" & ControlChars.NewLine &
            "       <mesa>00</mesa>" & ControlChars.NewLine &
            "       <precio>00</precio>" & ControlChars.NewLine &
            "   </venta>" & ControlChars.NewLine &
            "</Ventas>"
        Return xml
    End Function

    Public Function añadirNuevo(idVenta As Integer, array As String(), doc As XmlDocument)
        Dim fecha As Date = Date.Now.ToString("yyyy/MM/dd")
        Dim venta As XmlElement = doc.CreateElement("venta")

        Dim atributo As XmlAttribute = doc.CreateAttribute("id")
        atributo.Value = idVenta
        venta.Attributes.Append(atributo)

        atributo = doc.CreateAttribute("fecha")
        atributo.Value = fecha
        venta.Attributes.Append(atributo)

        Dim unidades As XmlElement = doc.CreateElement("unidades")
        unidades.InnerText = array(0)
        venta.AppendChild(unidades)

        venta.InnerXml = venta.InnerXml.Replace(ControlChars.NewLine + "    " & unidades.OuterXml, unidades.OuterXml & " " & ControlChars.NewLine + "    ")

        Dim mesa As XmlElement = doc.CreateElement("mesa")
        mesa.InnerText = array(1)
        venta.AppendChild(mesa)

        venta.InnerXml = venta.InnerXml.Replace(mesa.OuterXml, mesa.OuterXml & " " & ControlChars.NewLine + "    ")

        Dim precioTotal As XmlElement = doc.CreateElement("precio")
        precioTotal.InnerText = array(2)
        venta.AppendChild(precioTotal)

        venta.InnerXml = venta.InnerXml.Replace(precioTotal.OuterXml, precioTotal.OuterXml & " " & ControlChars.NewLine + "    ")
